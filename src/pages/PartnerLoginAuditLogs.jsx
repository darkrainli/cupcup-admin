...
const REASON_LABELS = {
  login_success: '登录成功',
  login_success_hash_upgraded: '登录成功并升级密码哈希',
...
export default function PartnerLoginAuditLogs() {
  ...
  const fetchLogs = async () => {
    ...
    try {
      let query = memFire
        ...
        .select('id, email, status, reason, failed_count, locked_until, ip_address, user_agent, created_at')
        .order('created_at', { ascending: false })
        .limit(300)
      if (status !== 'all') query = query.eq('status', status)
      if (email.trim()) query = query.ilike('email', `%${email.trim().toLowerCase()}%`)
      ...
      if (queryError) {
        ...
        if (msg.includes('relation') && msg.includes('does not exist')) {
          throw new Error('缺少商户登录审计表，请先执行 sql/partner_login_security_audit.sql')
        }
        throw queryError
      }
      setList(data || [])
      ...
      console.error('fetch partner login audit logs failed:', err)
      setList([])
  ...
  const exportCsv = () => {
    ...
    const headers = ['时间', '邮箱', '状态', '原因', '失败计数', '锁定到', 'IP', '终端信息']
    ...
    a.href = url
    a.download = `partner-login-audit-${new Date().toISOString().slice(0, 19).replace(/[:T]/g, '-')}.csv`
      ...
      <nav className="bg-cc-surface/80 backdrop-blur-sm border-b border-cc-border px-6 py-4 sticky top-0 z-40 flex items-center justify-between shadow-cc-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <div className="bg-teal-100 text-teal-700 p-2 rounded-cc"><Shield size={20} strokeWidth={1.5} /></div>
            <h1 className="text-lg font-semibold text-cc-neutral-800">商户登录审计</h1>
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        <div className="bg-cc-surface border border-cc-border rounded-cc-xl p-5 shadow-sm mb-5">
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <button
              ...
              className="bg-teal-100 text-teal-700 px-4 py-2 rounded-cc font-bold text-sm hover:bg-teal-200 disabled:opacity-50"
            >
              导出 CSV
            </button>
                ...
                <tbody>
                  {pageList.map((row) => (
                    <tr key={row.id} className="border-t border-cc-border align-top">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap font-semibold text-cc-neutral-800">{row.email || '-'}</td>
